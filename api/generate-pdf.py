...
def get_supabase(token):
    url = os.environ.get('SUPABASE_URL')
    key = os.environ.get('SUPABASE_ANON_KEY')
    req = urllib.request.Request(
        f"{url}/rest/v1/permits?token=eq.{token}&select=*",
...
def register_fonts():
    # Vercel Python 환경에서 폰트 경로
    candidates = [
        ('/var/task/fonts/NanumGothic.ttf', '/var/task/fonts/NanumGothicBold.ttf'),
        ('/usr/share/fonts/truetype/nanum/NanumGothic.ttf', '/usr/share/fonts/truetype/nanum/NanumGothicBold.ttf'),
    ]
    kr, krb = None, None
    for k, kb in candidates:
        if os.path.exists(k):
            kr, krb = k, kb
            break
    if not kr:
        raise Exception('나눔고딕 폰트를 찾을 수 없습니다. fonts/ 폴더에 NanumGothic.ttf 를 추가해주세요.')
    pdfmetrics.registerFont(TTFont('KR', kr))
    pdfmetrics.registerFont(TTFont('KRB', krb))
...
def sig_row(name='', date='', time='', sig_b64=None):
    ...
    if sig_b64:
        try:
            ...
            sig_cell = Image(img_io, width=50*mm, height=11*mm)
        except:
            pass
    return Table([[
...
def build_pdf(permit):
    fields = {}
    sec3   = {}
    sec4   = {}
    extra  = {}
    try: fields = json.loads(permit.get('form_fields') or '{}')
    except: pass
    try: sec3 = json.loads(permit.get('section3_data') or '{}')
    except: pass
    try: sec4 = json.loads(permit.get('section4_data') or '{}')
    except: pass
    try: extra = json.loads(permit.get('extra_fields') or '{}')
    except: pass
    ...
    story.append(mk([[p('')], [ni]], [TW],
        [('TOPPADDING',(0,0),(-1,-1),1),('BOTTOMPADDING',(0,0),(-1,-1),1),
         ('LEFTPADDING',(0,0),(-1,-1),0),('ALIGN',(0,1),(0,1),'RIGHT')]))
 
    story.append(one('본 양식(작업허가서 양식)은 작업 활동 관련하여, 상세 명시된 고위험군 작업에 대한 승인 및 전달 사항 등을 관리하기 위한 것이다.'))
 
    story.append(sh('Section 1: 일반 사항'))
    story.append(mk([[p('작업명:', FS, True), p(fields.get('s1-work') or permit.get('project_name',''), FS)]],[26*mm,TW-26*mm],[('BACKGROUND',(0,0),(0,0),GR)]))
    story.append(mk([[p('작업 장소:', FS, True), p(fields.get('s1-place',''), FS)]],[26*mm,TW-26*mm],[('BACKGROUND',(0,0),(0,0),GR)]))
    story.append(mk([[p('작업 업체:', FS, True), p(fields.get('s1-company') or permit.get('company_name',''), FS)]],[26*mm,TW-26*mm],[('BACKGROUND',(0,0),(0,0),GR)]))
    ...
    rg.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.4,LG),('TOPPADDING',(0,0),(-1,-1),1.5),('BOTTOMPADDING',(0,0),(-1,-1),1.5),('LEFTPADDING',(0,0),(-1,-1),4)]))
    ...
    story.append(mk([[p('본 작업허가서 발급이\n필요한 특정\n고위험군 작업:',FS,True,align=1),rr]],[28*mm,TW-28*mm],[('BACKGROUND',(0,0),(0,0),GR)]))
 
    story.append(sh('Section 2: 승인 요청 (작업 업체 관리 감독자 작성)'))
    story.append(one('본인은 고위험군 작업을 한 허가를 요청하는 사람으로써, 다음 사항을 확인한다.'))
    story.append(chk(['본인은 위험성 평가를 비롯하여 본 작업 활동과 관련하여 필요한 관리 양식들을 검토하였다.','본인은, 조치가 적절히 이루어졌는지 확실히 하기 위해 관계자와 협의하였다.','본인은, 첨부된 위험성 평가 및 해당 관리 양식에 따라 본 작업 활동을 조정 및 관리할 능력이 있다.','본인은 작업자 및 작업에 영향을 받는 사람의 건강 및 안전을 확보하기 위해 필요한 모든 조치를 취할것이다.','본인은, 작업 수행자가 위험성 평가 및 각 관리 양식은 물론 아래 Section 5 의 지시사항에 대한 설명을 들었고 해당 내용들을 이해했음을 확인하여야 한다.','본인은, 작업 활동 전체에 걸쳐 위험요소를 관찰하고 관리하여야 한다.','본인은, 당 작업 허가 승인 담당자에게 본 허가서를 검토하고, 등록 및 번호를 부여하도록 요청하는 바이다.']))
    story.append(sig_row(fields.get('s2-name',''), fields.get('s2-date',''), fields.get('s2-time',''), fields.get('sig2_img')))
 
    story.append(sh('Section 3: 작업업체 검토 (업무담당자 작성)'))
    story.append(one('본인은 모든 문서들을 검토하였고, 의견 제시했다.'))
    story.append(chk(['본인은, 위험성 평가 및 필요한 관리 양식을 포함하여 관련된 모든 문서들의 내용을 검토하였으며 작업 업체/작업자에게 의견을 제시하였다.','본인은, 작업 업체/작업자를 담당하는 자로서, 작업 방식과 제안된 관리방안들이 시행되는 것을 모니터링하여 BNCT 터미널 안전 기준이 달성될 수 있도록 할 것이다.','본인은, 해당 작업 구역의 관련자에게 작업 시행 구간, 완결해야 할 모든 작업 내역, 작업 관리감독 및 강화를 위해 수립된 절차들에 대한 정보를 알려주었다.']))
    ...
    story.append(chk(['본인은, 위험성 평가를 비롯하여 필요한 관리 양식을 포함한 모든 관련 문서 내용을 검토하였다.','본인은 당 작업 허가서를 등록하고, 작업 번호를 부여하였으며, 관련된 모든 문서 상에 해당 작업 번호를 기입하였다.']))
    story.append(sig_row(sec4.get('name','이병문'), sec4.get('date',''), sec4.get('time',''), sec4.get('sig')))
    story.append(mk([[p('제한 사항:', FS, True), p('본 작업 승인 사항은 아래 명시된 시점 혹은 일시 까지만 유효하다.', FS)]],[22*mm,TW-22*mm],[('BACKGROUND',(0,0),(0,0),GR)]))
    story.append(mk([[p('',FS),p('날짜:',FS,True),p('',FS),p('시간:',FS,True),p('',FS)]],[72*mm,14*mm,44*mm,14*mm,37*mm],[('BACKGROUND',(1,0),(1,0),GR),('BACKGROUND',(3,0),(3,0),GR)]))
    story.append(mk([[p('□  작업허가 연장 요청이 있을 경우, Section 7 을 작성하시오.', FS)]], [TW]))
    story.append(mk([[p('Page 1 of 2', 7, color=colors.Color(0.5,0.5,0.5), align=2)]], [TW], [('TOPPADDING',(0,0),(-1,-1),2)]))
 
    # ══ PAGE 2 ══
    story.append(PageBreak())
 
    t2 = Table([[p('작업허가서',7.5,True,W), p('작업허가서  양식 (계속)',14,True,NAVY)]],colWidths=[22*mm,159*mm])
    t2.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),NAVY),('LINEBELOW',(0,0),(-1,-1),1.5,NAVY),('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),4),('LEFTPADDING',(0,0),(-1,-1),5),('VALIGN',(0,0),(-1,-1),'MIDDLE')]))
    story.append(t2)
    story.append(sp(1))
 
    story.append(sh('Section 5: 작업허가/작업구역 출입관련 지시 사항 (작업자 작성)'))
    story.append(one('Note: 작업자는 반드시 아래 각각의 지시사항을 준수하여야 한다.'))
    story.append(chk(['본인의 역할 및 업무, 작업 범위에 대해 숙지하여야 한다.','위험성 평가 및 관리 양식을 읽고, 설명을 들었으며, 이를 이해하고 있어야 한다. 확실하지 않은 사항에 대해서는 질문하도록 한다. 만약 작업 전에 우려사항이 있다면 보고하도록 한다.','각 조 작업을 시작할 때마다 현장 감독자에게 알려야 하며, 작업허가/ 출입 관련 지시 및 허가를 받아 작업을 시작하도록 한다.','교대 근무를 할 경우, 작업 시작 전/종료 후 서명한다. (Section 7 참고)','작업 하는 동안 새로운 위험요소가 발견되었다면, 즉시 해당 작업 업체/작업자들의 현장 감독자와 기타 작업과 연관된 당사자들에게 통보한다.']))
    story.append(sp(1.5))
    ...
    ],[36*mm,TW-36*mm], rowHeights=[None,14*mm,None,None,12*mm])
    ...
    story.append(chk(['본인은, 위험성 평가를 비롯하여 필요한 관리 양식을 포함한 모든 관련 문서 내용을 검토하였다.','본 허가서를 작업 허가 목록에 수정 반영하였다. 작업 연장은 한번만 할 수 있으며, 당초 작업 완료일 기준일로부터 최대 7일까지 가능하다.']))
    ...
    story.append(mk([[p('Page 2 of 2', 7, color=colors.Color(0.5,0.5,0.5), align=2)]], [TW], [('TOPPADDING',(0,0),(-1,-1),3)]))
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if not token:
            ...
            self.end_headers()
            self.wfile.write(b'{"error":"token missing"}')
            return
 
        try:
            register_fonts()
            permit = get_supabase(token)
            if not permit:
            ...
            pdf_bytes = build_pdf(permit)
 
            self.send_response(200)
            self.send_header('Content-Type', 'application/pdf')
            self.send_header('Content-Disposition', 'attachment; filename="BNCT_permit.pdf"')
