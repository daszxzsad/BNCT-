...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try { extra = JSON.parse(permit.extra_fields || sec3.extraFields || '{}'); } catch(e){}
    ...
    await new Promise((resolve, reject) => {
      ...
      function row(label, value, yw, lw) {
        ...
        doc.font('Helvetica').text(value || '', M + lw + 3, yw + 4, { width: TW - lw - 6 });
        return yw + rh;
      }
      ...
      doc.fillColor('white').fontSize(8).font('Helvetica-Bold').text('작업허가서', M + 3, y + 6);
      doc.fillColor(NAVY).fontSize(18).font('Helvetica-Bold').text('작업허가서  양식', M + 65, y + 2);
      ...
      doc.fillColor('black').fontSize(7.5).font('Helvetica-Bold').text('제한 사항:', M + 3, y + 3);
      doc.font('Helvetica').text('본 작업 승인 사항은 아래 명시된 시점 혹은 일시 까지만 유효하다.', M + 73, y + 3, { width: TW - 76 });
      y += 14;
 
      doc.rect(M, y, TW, 14).stroke(LG);
      drawRect(M + 100, y, 40, 14, GRAY, LG);
      drawRect(M + 220, y, 40, 14, GRAY, LG);
      doc.font('Helvetica-Bold').text('날짜:', M + 103, y + 3);
      doc.text(extra.s4_limit_date || '', M + 145, y + 3);
      doc.text('시간:', M + 223, y + 3);
      doc.font('Helvetica').text(extra.s4_limit_time || '', M + 265, y + 3);
      y += 14;
      ...
      doc.fontSize(7).fillColor('#888').text('Page 1 of 2', M, y + 3, { width: TW, align: 'right' });
      ...
      doc.fillColor('white').fontSize(8).font('Helvetica-Bold').text('작업허가서', M + 3, y + 5);
      doc.fillColor(NAVY).fontSize(14).font('Helvetica-Bold').text('작업허가서  양식 (계속)', M + 65, y + 2);
      ...
      for (const [lbl, val, rh] of s6rows) {
        ...
        doc.fillColor('black').fontSize(7.5).font('Helvetica-Bold').text(lbl, M + 3, y + 3, { width: 95 });
        doc.font('Helvetica').text(val || '', M + 103, y + 3, { width: TW - 107 });
        y += rh;
      }
      y += 5;
      ...
      doc.fontSize(7).fillColor('#888').text('Page 2 of 2', M, y + 5, { width: TW, align: 'right' });
